...
namespace core {
	...
	static constexpr Vec2 Second_Player_Tank_Bullet_Firing_Positions[] = { {0.6f,0.0f},{0.0f,0.6f},{-0.6f,0.0f},{0.0f,-0.6f} };
	...
	void Game::update_2player(float delta_time) {
		if (player_tank.destroyed && player_lifes > 0) {
			player_respawn_timer -= delta_time;
			if (player_respawn_timer <= 0.0f) {
				player_respawn_timer = 0.0f;
		...
		if (player_lifes <=0){
			player_tank.position = {-1.0f,-1.0f};
			player_tank.shoot_cooldown = 1000.0f;
		}
		if (second_tank.destroyed && second_player_lifes > 0) {
			second_player_respawn_timer -= delta_time;
			if (second_player_respawn_timer <= 0.0f) {
				second_player_respawn_timer = 0.0f;
				second_tank.destroyed = false;
				second_tank.dir = Entity_Direction::Up;
				second_tank.position = eagle.position - Vec2{2.0f, 0.0f};
				second_player_lifes -= 1;
				second_player_invulnerability_timer = 5.0f;
				add_spawn_effect(second_tank.position);
			}
			return;
		}
		if(second_player_lifes<=0){
			second_tank.position = { -1.0f,-1.0f};
			second_tank.shoot_cooldown = 1000.0f;
		}
		if (player_tank.destroyed && player_lifes == 0 && second_player_lifes ==0 && second_tank.destroyed) {
			game_lose_timer -= delta_time;
			if (game_lose_timer <= 0.0f) {
				game_lose_timer = 0.0f;
				scene = Scene::Game_Over2;
			}
			return;
		}
		...
		second_player_invulnerability_timer -= delta_time;
		if (second_player_invulnerability_timer <= 0.0f) second_player_invulnerability_timer = 0.0f;


		player_tank.shoot_cooldown -= delta_time;
		second_tank.shoot_cooldown -= delta_time;
		if(player_tank.shoot_cooldown <= 0.0f) player_tank.shoot_cooldown = 0.0f;
		if(second_tank.shoot_cooldown <= 0.0f) second_tank.shoot_cooldown = 0.0f;
		...
		if ((platform->was_key_pressed(Keycode::Return) || platform->was_key_pressed(Keycode::Add))&& second_tank.shoot_cooldown <= 0.0f) {
			Bullet bullet{};
			bullet.fired_by_player = true;
			...
			bullets.push_back(bullet);
			second_tank.shoot_cooldown = Tank_Shoot_Cooldown;
		}
		...
		std::int32_t start_x_2 = std::int32_t((second_tank.position.x - Tank_Size.x / 2.0f + 0.1f) * 2.0f);
		std::int32_t start_y_2 = std::int32_t((second_tank.position.y - Tank_Size.y / 2.0f + 0.1f) * 2.0f);
		std::int32_t end_x_2 = std::int32_t((second_tank.position.x + Tank_Size.x / 2.0f - 0.1f) * 2.0f);
		std::int32_t end_y_2 = std::int32_t((second_tank.position.y + Tank_Size.y / 2.0f - 0.1f) * 2.0f);
	...
	void Game::update2_enemies(float delta_time) {
		if (remaining_enemy_count_to_spawn == 0 && enemy_tanks.size() == 0) {
			game_win_timer -= delta_time;
			if (game_win_timer <= 0.0f) {
				game_win_timer = 0.0f;
				scene = Scene::Outro_2player;
			}
		}
		...
		Rect player_rect = { player_tank.position.x - Tank_Size.x / 2.0f,player_tank.position.y - Tank_Size.y / 2.0f,1.0f,1.0f };
		Rect player2_rect = { second_tank.position.x - Tank_Size.x / 2.0f,second_tank.position.y - Tank_Size.y / 2.0f,1.0f,1.0f };
		Rect eagle_rect = { eagle.position.x - Eagle_Size.x / 2.0f,eagle.position.y - Eagle_Size.y / 2.0f,Eagle_Size.x,Eagle_Size.y };
		for (auto& enemy : enemy_tanks) {
			enemy.shoot_cooldown -= delta_time;
			if (enemy.shoot_cooldown <= 0.0f) enemy.shoot_cooldown = 0.0f;
			...
			if (enemy.dir == Entity_Direction::Right) {
				if (dir_change_could_happen) {
					bool did_happen = false;
					if ((down_vision_rect.overlaps(player_rect)|| down_vision_rect.overlaps(player2_rect) || down_vision_rect.overlaps(eagle_rect)) && chance_0_1_dist(random_engine) < 0.25f) {
						enemy.dir = Entity_Direction::Down;
						did_happen = true;
					}
					else if ((left_vision_rect.overlaps(player_rect) || left_vision_rect.overlaps(player2_rect) || left_vision_rect.overlaps(eagle_rect)) && chance_0_1_dist(random_engine) < 0.25f) {
						enemy.dir = Entity_Direction::Left;
						did_happen = true;
					}
					else if ((up_vision_rect.overlaps(player_rect) || up_vision_rect.overlaps(player2_rect) || up_vision_rect.overlaps(eagle_rect)) && chance_0_1_dist(random_engine) < 0.25f) {
						enemy.dir = Entity_Direction::Up;
						did_happen = true;
					}
					if (did_happen) {
				...
				else if ((right_vision_rect.overlaps(player_rect)|| right_vision_rect.overlaps(player2_rect)) && enemy.shoot_cooldown <= 0.0f) {
					Bullet bullet{};
					bullet.dir = enemy.dir;
					bullet.position = enemy.position + Player_Tank_Bullet_Firing_Positions[std::size_t(enemy.dir)];
					bullets.push_back(bullet);
				...
				if (dir_change_could_happen) {
					bool did_happen = false;
					if ((right_vision_rect.overlaps(player_rect)|| right_vision_rect.overlaps(player2_rect) || right_vision_rect.overlaps(eagle_rect)) && chance_0_1_dist(random_engine) < 0.25f) {
						enemy.dir = Entity_Direction::Right;
						did_happen = true;
					}
					else if ((left_vision_rect.overlaps(player_rect) || left_vision_rect.overlaps(player2_rect) ||left_vision_rect.overlaps(eagle_rect)) && chance_0_1_dist(random_engine) < 0.25f) {
				...
				else if ((down_vision_rect.overlaps(player_rect)|| down_vision_rect.overlaps(player2_rect)) && enemy.shoot_cooldown <= 0.0f) {
					Bullet bullet{};
					bullet.dir = enemy.dir;
					bullet.position = enemy.position + Player_Tank_Bullet_Firing_Positions[std::size_t(enemy.dir)];
					bullets.push_back(bullet);
				...
				if (dir_change_could_happen) {
					...
					else if ((down_vision_rect.overlaps(player_rect) || (down_vision_rect.overlaps(player2_rect)) || down_vision_rect.overlaps(eagle_rect)) && chance_0_1_dist(random_engine) < 0.25f) {
						enemy.dir = Entity_Direction::Down;
						did_happen = true;
					}
					else if ((up_vision_rect.overlaps(player_rect) || (up_vision_rect.overlaps(player2_rect) || up_vision_rect.overlaps(eagle_rect))) && chance_0_1_dist(random_engine) < 0.25f) {
				...
				else if ((left_vision_rect.overlaps(player_rect)|| left_vision_rect.overlaps(player2_rect)) && enemy.shoot_cooldown <= 0.0f) {
					Bullet bullet{};
					bullet.dir = enemy.dir;
					bullet.position = enemy.position + Player_Tank_Bullet_Firing_Positions[std::size_t(enemy.dir)];
					bullets.push_back(bullet);
				...
				else if ((up_vision_rect.overlaps(player_rect) || (up_vision_rect.overlaps(player2_rect)))&& enemy.shoot_cooldown <= 0.0f) {
					Bullet bullet{};
					bullet.dir = enemy.dir;
					bullet.position = enemy.position + Player_Tank_Bullet_Firing_Positions[std::size_t(enemy.dir)];
					bullets.push_back(bullet);
		...
		switch(scene) {
			...
			case Scene::Game_2player: {
				...
				for (auto& bullet : bullets) {
					...
					Rect player2_rect = { second_tank.position.x - Tank_Size.x / 2.0f,second_tank.position.y - Tank_Size.y / 2.0f,1.0f,1.0f };
					if (!second_tank.destroyed && bullet_rect.overlaps(player_rect) && !bullet.fired_by_player) {
						if (second_player_invulnerability_timer <= 0.0f) {
							add_explosion(second_tank.position, delta_time);
							second_tank.destroyed = true;
							bullet.destroyed = true;
							static constexpr float Player_Respawn_Time = 2.0f;
							second_player_respawn_timer = Player_Respawn_Time;
						}
						else add_explosion(second_tank.position, delta_time);
						continue;
					}
	...
	void Game::render(float delta_time) {
		...
		switch(scene) {
			...
			case Scene::Intro_2player: {
				...
				renderer->draw_sprite({ Background_Tile_Count_X / 2.0f - 0.75f,10.0f }, { 1.0f,1.0f }, 0.0f, entity_sprites, Second_Player_Tank_Sprite_Layer_Index);
				break;
			}
			case Scene::Game_2player: {
				render_map();
				...
				if (!second_tank.destroyed) {
					...
					if (second_player_invulnerability_timer > 0.0f) {
						renderer->draw_sprite({ second_tank.position.x,second_tank.position.y,0.5f }, Tank_Size, core::entity_direction_to_rotation(second_tank.dir), construction_place_marker);
					}
				}
				for (const auto& enemy_tank : enemy_tanks) {
					renderer->draw_sprite({ enemy_tank.position.x,enemy_tank.position.y,0.5f }, Tank_Size, core::entity_direction_to_rotation(enemy_tank.dir), entity_sprites, Enemy_Tank_Sprite_Layer_Index);
				...
				renderer->draw_sprite({ 0.25f,Background_Tile_Count_Y - 0.25f,1.0f }, { 0.5f,0.5f }, 0, entity_sprites, Second_Player_Tank_Sprite_Layer_Index);
				count = std::snprintf(text_buffer, sizeof(text_buffer) - 1, "x%" PRIu32, second_player_lifes);
				if (count > 0) renderer->draw_text({ 11.75f,Background_Tile_Count_Y  - 0.25f,1.0f }, { 0.5f,0.5f }, { 1,1,1 }, text_buffer);
